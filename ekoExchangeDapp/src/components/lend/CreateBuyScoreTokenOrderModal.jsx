...
const CreateBuyScoreTokenOrderModal = ({ setModalOn, setChoice }) => {
    const viewFacetAbi = viewFacetAbiz;
    const buyFacetAbi = buyFacetAbiz
  ...
  async function getAcceptedEkoStables() {
    const viewFacet = new ethers.Contract(exchangeAddress, viewFacetAbi, provider)
    ...
    async function approve(){
        if(isConnected){
        try{
          const ekoStable = new ethers.Contract(selectedES, erc20ABI, signer);
          let app = await ekoStable.approve(exchangeAddress, givingAmount);
          //setIsLoading(true)
          await app.wait();
          notyf.success(`${selectedES} approved succesfully`)
          //setIsApproved(true)
        } catch{
          notyf.error(`Error while approving ${selectedES}`)
        }finally{
          //setIsLoading(false)
        }
      } else{
      ...
      async function createBuyOrder(){
        if(isConnected){
          try{
            ...
            console.log('facet created')
            let ex = await buyFacet.createBuyScoreTokensOrder(selectedES, givingAmount, selectedST, requestingAmount)
            console.log('function called')
            await ex.wait()
            notyf.success('Order created succesfully')
          } catch {
            notyf.error('Error while creating the order')
          }
        } else {
          notyf.error("Not connected")
        }
                  ...
                  <div className="p-6 space-y-6">
                    <div className="grid grid-cols-6 gap-6">
                        <div className="col-span-6 sm:col-span-3">
                            <label for="first-name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select Score Token</label>
                            {/* Dropdown menu */}
        ...
        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-black shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            ...
            "Fetching"            
                  ...
                  <Dropdown>
      <Dropdown.Button flat>Choose the Score Token you want to buy</Dropdown.Button>
        ...
        {acceptedScoreTokens ?  
            acceptedScoreTokens.map((scoreToken) =>{
              ...
              : null}
                        ...
                        <div className="col-span-6 sm:col-span-3">
                            <label for="last-name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Input requesting ST Amount</label>
                        ...
                        <div className="col-span-6 sm:col-span-3">
                            <label for="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select Eko Stable</label>
                            {/* Dropdown menu */}
...
<Dropdown>
      <Dropdown.Button flat>Choose the ekostable you are paying with</Dropdown.Button>
      <Dropdown.Menu aria-label="Static Actions"
        ...
        {acceptedEkoStables ?  
            acceptedEkoStables.map((ekoStable) =>{
              ...
              : null}
                        ...
                        <div className="col-span-6 sm:col-span-3">
                            <label for="phone-number" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Input ES giving amount</label>
                ...
                <div className="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                    ...
                    <button onClick={()=> createBuyOrder()} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Create BUY score token order</button>
...
export default CreateBuyScoreTokenOrderModal