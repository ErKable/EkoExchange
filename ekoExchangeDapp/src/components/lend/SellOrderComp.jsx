...
export default function SellOrderComp({order}) {
         // new state variables
    ...
    async function getTokensName(){
        const ScoreToken = new ethers.Contract(order.givingToken, erc20ABI, provider)
        let scoreName = await ScoreToken.name()
        console.log('scoreName', scoreName)
        setEkoStableName(scoreName)

        const EkoStable = new ethers.Contract(order.requestingToken, erc20ABI, provider)
        let ekoName = await EkoStable.name()
        console.log('ekoName', ekoName)
  ...
  return (
    ...
    <td className="px-6 py-4">
        ...
        <a type="button" data-modal-target="editUserModal" data-modal-show="editUserModal" className="font-medium text-blue-600 dark:text-blue-500 hover:underline" onClick={clicked}><button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Buy</button></a> 
    </td>
</tr>
{/* {choice && 
    ...
    {modalOn && <Modal setModalOn={setModalOn} setChoice={setChoice} givingToken={scoreTokenName} 
